...
namespace FireLand
{
	...
	hrs::unexpected_result DataBuffer::SyncAndWrite(std::uint32_t rounding_item_count,
...
#error LOL! WHY WE USE TRANSFER IF DATABUFFER IS HOST_VISIBLE ITSELF?!
		...
		{
			const hrs::block<vk::DeviceSize> blk(data_item_req.size, remove.offset * data_item_req.size);
		...
		if(free_count < queue.GetAdds().size())
		{
			...
			vk::DeviceSize new_size = hrs::round_up_size_to_alignment(old_buffer.size + append_size,
																	  data_item_req.size);
			...
			const TransferBufferOpRegion region({old_buffer.size, 0},
												0,
												0);
			...
			transfer.AddWaitFunction([old_buf = std::move(old_buffer),
									  allocator = parent_device->GetAllocator()]()
			{
		...
		regions.reserve(queue.GetAdds().size() + queue.GetUpdates().size() + queue.GetUpdatedAdds().size());
		...
		for(auto &upd : queue.GetUpdates())
		{
			...
			const TransferBufferOpRegion region(upd.data_block,
												upd.offset * data_item_req.size + upd.in_data_buffer_offset,
												datas.size() - 1);

			regions.push_back(region);
		}

		auto unexp_exp = transfer.CopyBuffer(bounded_buffer_size.bounded_buffer.buffer,
											 datas,
											 regions,
											 1);
	...
	hrs::unexpected_result DataBuffer::recreate_buffer(vk::DeviceSize size,
													   const std::function<NewPoolSizeCalculator> &calc)
	{
		destroy_buffer();
