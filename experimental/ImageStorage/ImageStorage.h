...
namespace FireLand
{
	struct ImageGroupKey
	{
		...
		ImageGroupKey(std::string_view _name,
			...
			: name(_name), calc(_calc), allocation_variants(_allocation_variants) {}

		ImageGroupKey(std::string_view _name,
					  const std::function<NewPoolSizeCalculator> &_calc,
					  std::vector<MemoryPropertyOpFlags> &&_allocation_variants)
			: name(_name), calc(_calc), allocation_variants(std::move(_allocation_variants)) {}
	};
};
...
namespace FireLand
{
	...
	public:
		...
		hrs::expected<ImagesMap::iterator, hrs::error>
		AddImage(ImageGroups::iterator group_it,
		...
		std::optional<ImagesMap::iterator> GetImage(std::size_t group_hash,
													const std::filesystem::path &path) noexcept;
		std::optional<ImagesMap::const_iterator> GetImage(std::string_view group_name,
														  const std::filesystem::path &path) const noexcept;
		std::optional<ImagesMap::const_iterator> GetImage(std::size_t group_hash,
														  const std::filesystem::path &path) const noexcept;
