...
namespace LuaWay
{
    namespace detail
    {
        [[noreturn]] inline void emit_error(lua_State* state, std::string_view message) noexcept
        {
...
#warning ADD SYNC FOR REF.state because it can be called from coroutine!!! -> CHECK LUA_RIDX_MAINTHREAD
            ...
            {
                func(std::forward<Args>(Stack<std::remove_cvref_t<Args>>::Retrieve(
                    state,
                    static_cast<int>(Indices) - static_cast<int>(sizeof...(Args))))...);

                return 0;
            }
            else
            {
                Stack<std::remove_cvref_t<R>>::Push(
                    state,
                    func(std::forward<Args>(Stack<std::remove_cvref_t<Args>>::Retrieve(
                        state,
                        static_cast<int>(Indices) - static_cast<int>(sizeof...(Args))))...));
        ...
        int call_member(lua_State* state, C* obj, std::index_sequence<Indices...>) noexcept
        {
            if constexpr(std::same_as<R, void>)
            {
                (obj->*func)(std::forward<Args>(Stack<std::remove_cvref_t<Args>>::Retrieve(
                    state,
                    static_cast<int>(Indices) - static_cast<int>(sizeof...(Args))))...);
        ...
        int function_wrapper(lua_State* state) noexcept
        {
            ...
            if constexpr(std::same_as<C, void>) //plain
            {
                if(stack_args_count < sizeof...(Args))
                    emit_error(state,
                               std::format("Too few arguments! Expected = {} "
                                           "but received = {}!",
                                           sizeof...(Args),
                                           stack_args_count));

                return call_plain<func, R, Args...>(state,
            ...
            else //class member
            {
                ...
                C* obj =
                    static_cast<C*>(lua_touserdata(state, -static_cast<int>(sizeof...(Args) + 1)));
                return call_member<func, C, R, Args...>(
                    state,
                    obj,
                    std::make_index_sequence<sizeof...(Args)>{});
            }
        ...
        int constructor_wrapper(lua_State* state) noexcept
        {
            ...
            if(stack_args_count < sizeof...(Args) + 1)
                emit_error(state,
                           std::format("Too few arguments! Expected = {} but received = {}!",
                                       sizeof...(Args) + 1,
                                       stack_args_count));

            [state]<std::size_t... Indices>(std::index_sequence<Indices...>)
            {
                Stack<std::remove_cvref_t<C>>::Push(
                    state,
                    C(std::forward<Args>(Stack<std::remove_cvref_t<Args>>::Retrieve(
                        state,
                        static_cast<int>(Indices) - static_cast<int>(sizeof...(Args))))...));
                //mt, args, obj
                //args = 2
    ...
    CFunction WrapFunction() noexcept
    {
        ...
        using f_traits =
            detail::select_func_class_tratis<func,
                                             hrs::is_class_member_v<decltype(func)>>::f_traits;

        if constexpr(!std::same_as<typename f_traits::return_type, void>)
            static_assert(Pushable<typename f_traits::return_type>,
                          "Return type must be Pushable!");
