...
namespace FireLand
{
	...
	struct AllocatorError
	{
		 ...
		 */
		constexpr bool IsDesiredUnsupported() const noexcept
		{
	...
	public:
		...
		/**
		 * @brief GetSupportedMemoryTypesMask
		 * @param flags flags which is checked for support
		 * @return mask with supported memory types
		 */
		constexpr auto GetSupportedMemoryTypesMask(vk::MemoryPropertyFlags flags) const noexcept;
	...
	constexpr auto Allocator::GetSupportedMemoryTypesMask(vk::MemoryPropertyFlags flags) const noexcept
	{
		using MaskType = decltype(std::declval<vk::MemoryRequirements>().memoryTypeBits);
		MaskType mask = 0;
		for(std::size_t i = 0; i < memory_types.size(); i++)
			if(memory_types[i].IsSatisfyAny(flags))
				mask |= static_cast<MaskType>(1 << i);
