...
namespace GeometryParser
{
	...
	private:
		...
		parse_result parse_face_element(std::string_view str, const face_parse_slashes &slashes) noexcept;

		hrs::expected<VectorElement, parse_result> parse_vertex(std::string_view str) noexcept;
		hrs::expected<VectorElement, parse_result> parse_texture_coordinates(std::string_view str) noexcept;
