...
namespace hrs
{
    namespace detail
    {
        template<hrs::static_string str,
                 std::size_t Index,
                 decltype(str)::char_t value,
                 decltype(str)::char_t... values>
        ...
        constexpr std::size_t trim_find_index() noexcept
        {
            if constexpr(dir_front)
            {
                if constexpr(Index == str.size)
                    return Index;
                else if constexpr(trim_check_value<str, Index, values...>())
            ...
            else
            {
                if constexpr(Index == 0)
                    return Index;
                else if constexpr(trim_check_value<str, Index - 1, values...>())
                    return trim_find_index<str, Index - 1, dir_front, values...>();
                else
                    return Index;
            }
    ...
    constexpr auto trim() noexcept
    {
        ...
        else
        {
            constexpr std::size_t index =
                detail::trim_find_index<str,
                                        detail::select_trim_start_index<str.size, dir_front>(),
                                        dir_front,
                                        values...>();
            if constexpr(dir_front)
        ...
        constexpr static auto next() noexcept
        {
            ...
            else
            {
                ...
                constexpr auto value =
                    it.haystack.template substr<fit.value + fit.needle.size,
                                                it.value - (fit.value + fit.needle.size)>();
    ...
    template<static_string _haystack,
             static_string _start,
             static_string _end,
    ...
    struct find_bound_cursor
    {
        ...
        constexpr static auto value =
            detail::find_bound_cursor_set_value<_haystack, _Start_Index, _End_Index>();
        ...
        constexpr static auto next() noexcept
        {
            ...
            else
            {
                ...
                if constexpr(start_index == haystack_t::npos)
                {
                    ...
                    return find_bound_cursor<_haystack,
                                             _start,
                                             _end,
                                             haystack_t::npos,
                                             end_index>{};
