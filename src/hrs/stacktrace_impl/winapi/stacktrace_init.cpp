    ...
    namespace winapi
    {
        namespace detail
        {
                ...
                : symbol_info(nullptr)
            {
                ...
                if(DuplicateHandle(target_process, target_process, target_process,
							   &dup_process, 0, FALSE, DUPLICATE_SAME_ACCESS) == FALSE)
                    return;

                auto cleanup = hrs::scoped_call([dup_process, sym_inited, _symbol_info]()
                {
                    if(_symbol_info)
				...
				_symbol_info =
					static_cast<PSYMBOL_INFO>(std::calloc(1, sizeof(SYMBOL_INFO) + sizeof(char) * MAX_SYM_NAME + 1));
