	...
	namespace winapi
	{
		...
		std::string frame::get_symbol_info(bool get_only_symbol_name, bool demangle) const
		{
			...
			if(get_only_symbol_name)
				return std::string(stacktrace_init_var.get_symbol_info()->Name, stacktrace_init_var.get_symbol_info()->NameLen);
			else
			{
				HMODULE module_addr;
				auto get_module_res = GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS,
				...
				auto count = GetModuleFileNameA(module_addr, module_name.data(), module_name.size());
