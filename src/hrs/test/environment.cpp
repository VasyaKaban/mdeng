...
namespace hrs
{
	namespace test
	{
		...
		void
		environment::config::set_output_end_function(std::function<end_output_f> &&_end_output_function)
		{
		...
		void
		environment::config::set_ignore_group_names(ignore_group_names_container &&_ignore_group_names)
		{
			ignore_group_names = std::move(_ignore_group_names);
		}
		...
		void environment::add_test(std::function<void ()> &&func,
								   const test_config &t_cfg)
		{
			test_data test(std::move(func), t_cfg.get_name(), t_cfg.get_tag(), t_cfg.get_properties());
			add_test(std::move(test), t_cfg.get_group());
		}

		void environment::add_test(void (*func)(),
								   const test_config &t_cfg)
		{
		...
		void environment::run()
		{
			...
			{
				bool ignore_group = cfg.ignore_group_names.find(group_name) != cfg.ignore_group_names.end();
