        ...
        constexpr auto retrieve_namespace_and_name() noexcept
        {
            ...
            else
            {
                constexpr auto make_pair =
                    []<std::size_t... Indices>(const auto& namespaces,
                                               std::index_sequence<Indices...>)
                {
                    return std::pair{
                        std::tuple{std::get<Indices>(namespaces)...},
                        std::get<std::tuple_size_v<std::remove_cvref_t<decltype(namespaces)>> - 1>(
                            namespaces)};
                };
                ...
                {
                    constexpr auto seq =
                        std::make_index_sequence<std::tuple_size_v<decltype(nss)> - 2>{};
                    constexpr auto create_index_seq =
                    ...
                    return make_pair(
                        nss,
                        std::make_index_sequence<std::tuple_size_v<decltype(nss)> - 1>{});
            }
        }
    };
};
