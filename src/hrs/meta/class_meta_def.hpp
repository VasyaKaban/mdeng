...
#define HRS_REFL_BEGIN(NAME, ...) \
    ...
    { \
        template<> \
        struct class_meta<NAME> \
            : detail::class_meta_base<NAME, #NAME, meta_attributes<__VA_ARGS__>> \
        {
#define HRS_REFL_END() \
    static_assert(std::is_class_v<refl_class> || \
                      (std::is_union_v<refl_class> && parents::COUNT == 0), \
...
#define HRS_REFL_MEMBER_FIELDS_BEGIN() using member_fields = hrs::variadic <
...
#define HRS_REFL_USINGS_BEGIN() using using_fields = hrs::variadic <
...
#define HRS_REFL_STATIC_FIELDS_BEGIN() using static_fields = hrs::variadic <

#define HRS_REFL_STATIC_FIELDS_END() > ;

#define HRS_REFL_PARENT_CLASSES(...) using parents = hrs::variadic<__VA_ARGS__>;
