	...
	namespace detail
	{
		...
		struct node : node_base<V, Keys...>
		{
			...
			template<typename T, typename ...TKeys>
				requires
					(sizeof...(TKeys) == sizeof...(Keys)) &&
					std::constructible_from<std::tuple<Keys...>, TKeys...> &&
					std::constructible_from<V, T>
			constexpr node(T &&_data, TKeys &&..._keys)
				noexcept(std::is_nothrow_constructible_v<std::tuple<Keys...>, TKeys...> &&
			...
			template<typename T, typename ...TKeys>
				requires
					...
					std::constructible_from<V, T>
			constexpr node(T &&_data, std::tuple<TKeys...> &&_keys)
				noexcept(std::is_nothrow_constructible_v<std::tuple<Keys...>, std::tuple<TKeys...>> &&
						 std::is_nothrow_constructible_v<V, T>)
				: keys(std::move(_keys)),
				  data(std::forward<T>(_data))
			{
		...
		std::uint8_t calculate_height(const node<V, Keys...> *node, std::size_t index) noexcept
		{
			...
			std::size_t right_height = get_node_height(node->children[index].right->to_node(), index);
		...
		rotation_type select_rotation(const node<V, Keys...> *node, std::size_t index) noexcept
		{
			...
			std::uint8_t left_height = get_node_height(node->children[index].left->to_node(), index);
			std::uint8_t right_height = get_node_height(node->children[index].right->to_node(), index);
			...
			if(left_height > right_height)
			{
				...
				std::uint8_t left_height = get_node_height(left_node->children[index].left->to_node(),
														   index);
				std::uint8_t right_height = get_node_height(left_node->children[index].right->to_node(),
															index);
			...
			else
			{
				...
				std::uint8_t left_height = get_node_height(right_node->children[index].left->to_node(),
														   index);
				std::uint8_t right_height = get_node_height(right_node->children[index].right->to_node(),
															index);
