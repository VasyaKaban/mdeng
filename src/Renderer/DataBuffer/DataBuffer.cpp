...
namespace FireLand
{
	...
	hrs::error DataBuffer::Recreate(std::uint32_t init_item_count)
	{
		...
		vk::DeviceSize buffer_size = hrs::round_up_size_to_alignment(init_item_count,
																	 rounding_item_count) *
									 data_item_req.size;
	...
	{	
		...
		{
			const hrs::block<vk::DeviceSize> blk(data_item_req.size, remove.index * data_item_req.size);
		...
		if(free_count < queue.GetAdds().size())
		{
			...
			vk::DeviceSize buffer_size = hrs::round_up_size_to_alignment(total_count,
																		 rounding_item_count) *
										 data_item_req.size;
			auto err = recreate_buffer(buffer_size);
			if(err)
		...
		for(auto &add : queue.GetAdds())
		{
			...
			*add.output_write = offset;
			std::copy_n(std::execution::unseq,
						add.data.GetData(),
						data_item_req.size,
						bounded_buffer_size.bounded_buffer.GetBufferMapPtr() + blk_pair.first.offset);
		}
