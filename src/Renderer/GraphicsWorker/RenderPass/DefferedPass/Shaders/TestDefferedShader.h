...
namespace FireLand
{
	class TestDefferedShader : public DefferedPassShader, public hrs::non_copyable
	{
		void init(vk::Pipeline _pipeline, vk::PipelineLayout _pipeline_layout) noexcept;

	public:

		TestDefferedShader(vk::Device _parent_device) noexcept;
		virtual ~TestDefferedShader() override;
		TestDefferedShader(TestDefferedShader &&def_sh) noexcept;
		TestDefferedShader & operator=(TestDefferedShader &&def_sh) noexcept;
