#include "TestDefferedShader.h"
#include "../../../../../hrs/debug.hpp"
#include "../../../../../Vulkan/UnexpectedVkResult.hpp"

namespace FireLand
{
	void TestDefferedShader::init(vk::Pipeline _pipeline, vk::PipelineLayout _pipeline_layout) noexcept
	{
		pipeline = _pipeline;
		pipeline_layout = _pipeline_layout;
	}

	TestDefferedShader::TestDefferedShader(vk::Device _parent_device) noexcept
		:parent_device(_parent_device)
	{
		hrs::assert_true_debug(_parent_device, "Parent device isn't created yet!");
	}

	TestDefferedShader::~TestDefferedShader()
	{
		TestDefferedShader::Destroy();
	}

	TestDefferedShader::TestDefferedShader(TestDefferedShader &&def_sh) noexcept
		: parent_device(def_sh.parent_device),
		  pipeline(def_sh.pipeline),
		  pipeline_layout(def_sh.pipeline_layout)
	{
		def_sh.pipeline = VK_NULL_HANDLE;
		def_sh.pipeline_layout = VK_NULL_HANDLE;
	}

	TestDefferedShader & TestDefferedShader::operator=(TestDefferedShader &&def_sh) noexcept
	{
		Destroy();
	...
	void TestDefferedShader::Destroy()
	{
		if(!TestDefferedShader::IsCreated())
			return;
	...
	bool TestDefferedShader::IsCreated() const noexcept
	{
		return pipeline;
	}

	vk::Pipeline TestDefferedShader::GetPipeline() const noexcept
	{
		return pipeline;
	}

	vk::PipelineLayout TestDefferedShader::GetPipelineLayout() const noexcept
	{
		return pipeline_layout;
	}

	bool TestDefferedShader::IsSceneIndependent() const noexcept
	{
		return true;
	}

	hrs::unexpected_result
	TestDefferedShader::Recreate(const std::map<vk::ShaderStageFlagBits, ShaderInfo> shader_infos,
								 ...
								 const vk::Extent2D &resolution)
	{
		...
		const std::array push_constants_ranges =
		{
			vk::PushConstantRange(vk::ShaderStageFlagBits::eVertex, 0, 64/*mat4x4*/)
		};
	...
	void TestDefferedShader::Bind(vk::CommandBuffer command_buffer,
	...
	void TestDefferedShader::PushConstants(vk::CommandBuffer command_buffer,
