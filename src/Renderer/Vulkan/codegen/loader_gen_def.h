...
#define FIRE_LAND_LOADER_BEGIN_INSTANCE(NAME, INIT_NAME) \
    LoaderInitResult NAME::INIT_NAME( \
        VkInstance instance, \
        PFN_vkGetInstanceProcAddr global_vkGetInstanceProcAddr) noexcept \
    { \
        vkGetInstanceProcAddr = reinterpret_cast<PFN_vkGetInstanceProcAddr>( \
            global_vkGetInstanceProcAddr(instance, "vkGetInstanceProcAddr")); \
        if(!vkGetInstanceProcAddr) \
            return std::monostate{}; \
\
        vkGetInstanceProcAddr = reinterpret_cast<PFN_vkGetInstanceProcAddr>( \
...
#define FIRE_LAND_LOADER_BEGIN_DEVICE(NAME, INIT_NAME) \
    ...
    { \
        vkGetDeviceProcAddr = reinterpret_cast<PFN_vkGetDeviceProcAddr>( \
            instance_vkGetDeviceProcAddr(device, "vkGetDeviceProcAddr")); \
...
#define FIRE_LAND_LOADER_FUNCTION_ALT(NAME) \
    if(required_function = reinterpret_cast<PFN_##NAME>(resolve_function(resolver, #NAME)), \
       !required_function)
