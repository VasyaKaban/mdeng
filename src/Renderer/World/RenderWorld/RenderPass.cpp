...
namespace FireLand
{
    ...
    std::pair<vk::CommandBuffer, vk::Result> RenderPass::Render(std::uint32_t frame_index,
    ...
    void RenderPass::NotifyRemoveShaderObjectData(const Shader* shader, std::uint32_t index)
    {
        auto it = shaders_search.find(shader);
        if(it == shaders_search.end())
            return;
                                                  ...
                                                  std::uint32_t* subscriber_ptr)
    {
        ...
        it->second.it->second->NotifyNewRenderGroupInstance(render_group,
                                                            data_index,
                                                            subscriber_ptr);
    }
    ...
    RenderPass::add_shader(Shader* shader, std::uint32_t subpass, std::size_t priority)
    {
    ...
    void RenderPass::shader_free_command_buffers(
        std::span<const vk::CommandBuffer> shader_buffers) noexcept
    {
