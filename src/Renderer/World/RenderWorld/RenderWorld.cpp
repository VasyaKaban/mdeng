...
namespace FireLand
{
    ...
    void RenderWorld::NotifyRemoveShaderObjectData(const Shader* shader, std::uint32_t index)
    {
        const RenderPass* renderpass = shader->GetParentRenderPass();
        auto it = renderpasses_search.find(renderpass);
        if(it == renderpasses_search.end())
                                             ...
                                             bool _enabled)
    {
        ...
        return it->second->second
            ->AddRenderGroup(shader, mtl, mesh, init_size_power, rounding_size, _enabled);
    }
    ...
    RenderWorld::GetNewPoolSizeCalculator() const noexcept
    {
        return calc;
    }
