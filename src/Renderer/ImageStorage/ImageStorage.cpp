...
namespace FireLand
{
	...
	std::optional<ImageStorage::ImageGroups::const_iterator> ImageStorage::GetGroup(std::size_t hash) const
	{
		auto it = image_groups.find(hash);
		if(it == image_groups.end())
			return {};
						   ...
						   const vk::ImageCreateInfo &info)
	{
		...
		auto image_exp = Image::Create(this, group_it->first.allocation_variants, group_it->first.calc, info);
		if(!image_exp)
	...
	std::optional<ImageStorage::ImagesMap::iterator>
	ImageStorage::GetImage(std::size_t group_hash,
						   const std::filesystem::path &path) noexcept
	{
