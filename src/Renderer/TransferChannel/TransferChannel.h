...
namespace FireLand
{
	...
	public:
		...
		VkResult WaitFence(std::uint64_t timeout = std::numeric_limits<std::uint64_t>::max()) noexcept;
		...
		hrs::error
		CopyBuffer(VkBuffer dst_buffer,
		...
		hrs::error
		CopyImageSubresource(VkImage dst_image,
	...
	private:
		...
		VkDeviceSize image_data_size(const VkExtent3D &extent, VkDeviceSize block_size) const noexcept;
